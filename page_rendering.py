...
def get_prefered_content_language():
    return request.args.get("prefered_content_language", default=DEFAULT_LANGUAGE, type=str)

def get_theme():
    return request.args.get("theme", default=DEFAULT_THEME, type=str)

def load_yaml_data(language, file_name):
    path = f"./data/{file_name}" if language is None else f"./data/{language}/{file_name}"
    with open(path, "r") as file:
        data = yaml.safe_load(file)
    return data

def join_base_with_content(ui_language, content):
    template = f'''{{% extends "{ui_language}/base.html" %}}\n 
...
def get_yaml_data(yaml_data, content_language, find_other_language=False):
    ...
    yaml_data_exists_for_language = os.path.exists(f"data/{content_language}/{yaml_data}")
    ...
    if yaml_data_exists_for_language:
        ...
        if find_other_language and len(available_languages_for_yaml) > 0:
            ...
            if prefered_language in available_languages_for_yaml:
                ...
                return load_yaml_data(available_languages_for_yaml[0], yaml_data), available_languages_for_yaml[0]
            
        else:
            warn(f"Yaml data file '{yaml_data}' not found for language '{content_language}'.")
...
def get_page_not_in_language_message(template_name, language, yaml_file_name=None):
    available_languages_for_template = get_available_languages_for_file("templates", template_name)
    if not available_languages_for_template and yaml_file_name is not None:
        available_languages_for_template = get_available_languages_for_file("data", yaml_file_name)
...
def render_page(template_name, language, yaml_data_name=None):
    available_languages_for_template = get_available_languages_for_file("templates", template_name)
    ...
    if template_exists_in_language:
        ...
        if yaml_data_name is not None:
            ...
            data, data_language = get_yaml_data(yaml_data_name, content_language, find_other_language=True)
    ...
    if yaml_data_name is None:
        ...
        content = render_template(template_to_render, lang=language, theme=get_theme(), data=data)


    if content_language != language or data_language != language:
        content = get_page_not_in_language_message(template_name, language, yaml_data_name) + content

    
    return join_base_with_content(language, content)
